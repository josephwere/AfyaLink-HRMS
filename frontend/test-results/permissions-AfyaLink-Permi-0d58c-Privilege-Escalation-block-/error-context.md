...
- Name: permissions.spec.ts >> AfyaLink Permission and Auth Access Control tests >> Nurse token should fail to access super admin APIs (Privilege Escalation block)
- Location: tests/e2e/permissions.spec.ts:43:7

# Error details

```
Error: Unable to obtain auth token for NURSE: {"message":"Too many sign-in or recovery attempts. Wait a few minutes and try again.","code":"AUTH_SENSITIVE_RATE_LIMITED"}
```
...
> 194 |     throw new Error(`Unable to obtain auth token for ${role}: ${JSON.stringify(body)}`);
  ...
  239 |   }
  240 | 
  241 |   headers['X-Hospital'] = fallbackHospitalId;
  242 |   headers['X-Hospital-Id'] = fallbackHospitalId;
  243 |   const expiresAt = Date.now() + AUTH_CACHE_TTL_MS;
  244 |   authCache.set(cacheKey, { headers: { ...headers }, expiresAt });
  245 |   await persistAuthState(role, apiURL, headers, expiresAt);
  246 |   return { ...headers };
  247 | }
  248 | 
```