...
export async function createStripeIntent(req, res) {
  ...
  try {
    const hospital = req.user?.hospital || metadata?.hospital || null;
    const intent = await stripe.paymentIntents.create({
      amount: Math.round(amount),
      currency,
      metadata
    });
    await Transaction.create({ provider: 'stripe', reference: intent.id, amount, currency, status: 'pending', meta: metadata, hospital });
...
export async function mpesaSTKPush(req, res) {
  ...
  try {
    ...
    await Transaction.create({ provider: 'mpesa', reference: j.CheckoutRequestID || 'mpesa-'+Date.now(), amount, status:'pending', meta: j, hospital });
    res.json(j);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
...
export async function flutterwaveInit(req, res) {
  ...
  try {
    ...
    const j = await r.json();
    await Transaction.create({ provider: 'flutterwave', reference: j.data?.id || j.data?.link, amount, status: 'pending', meta: j, hospital });
    res.json(j);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
